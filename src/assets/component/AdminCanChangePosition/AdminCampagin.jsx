...
const AdminCampagin = () => {
  ...
  const fetchData = async () => {
    try {
      // Fetch media data
      const mediaResponse = await fetch('http://localhost:8081/campaigns');
      const mediaData = await mediaResponse.json();
      setMediaData(mediaData);

      // Fetch layout data
      const layoutResponse = await fetch('http://localhost:8081/layouts');
  ...
  const saveLayoutToDB = async () => {
    try {
      const response = await fetch('http://localhost:8081/layouts', {
        method: 'POST',
        headers: {
